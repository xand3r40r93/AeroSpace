...
You can bootstrap your custom config by coping the default config from the app installation
...
Accordion padding is configurable via following options:

* `accordion-padding` - Default padding size for all containers
* `accordion-padding-root` - Optional custom padding size for root container (if not set, uses `accordion-padding` value)
* `accordion-padding-root-edges` - Controls edge padding behavior for root container only:
** When `true` (default): First element has no left padding, last element has no right padding
** When `false`: All elements in root container have equal padding on both sides
** Note: Non-root containers always have padding for first and last elements
